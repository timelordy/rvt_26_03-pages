(() => {
  ...
  function lessonButton(lesson) {
    ...
    return `<button class="live-lesson-link${active ? ' active' : ''}" data-action="guide-lesson" data-id="${lesson.id}" aria-current="${active ? 'page' : 'false'}"><span>Пара ${String(lesson.number).padStart(2, '0')} → этап ${String(lesson.stageId).padStart(2, '0')}</span><b>${esc(lesson.title)}</b></button>`;
  }
  ...
  function liveGuidePage() {
    ...
    if (lesson.id === 'start' && window.S40Lesson01) {
      return `<article class="live-lessons"><header class="live-lessons-hero"><div><span class="eyebrow">Первый семестр</span><h2>Пары и этапы проекта</h2><p>Каждая пара связана с этапом того же номера: на занятии разбираем инструменты, а ниже сразу видно, что должно остаться в вашем RVT.</p></div><div class="live-lessons-count"><b>Открыто: 2 пары = 2 этапа</b><br><span class="muted">остальные откроются по ходу курса</span></div></header><div class="live-lessons-layout"><nav class="live-lessons-nav" aria-label="Открытые пары">${LIVE_LESSONS.map(lessonButton).join('')}</nav><section class="live-lessons-panel">${window.S40Lesson01.render({assets:'assets/lesson-01/'})}${projectStageBlock(lesson)}${lessonPager(lesson)}</section></div></article>`;
    }
    ...
    return `<article class="live-lessons">
      <header class="live-lessons-hero">
        ...
        <div class="live-lessons-count"><b>Открыто: 2 пары = 2 этапа</b><br><span class="muted">остальные откроются по ходу курса</span></div>
      </header>
